...
func (c *Connected) OnEnter() {
	// We grab the DBTX instance from the client and access one of the methods generated by sqlc
	/*user, err := c.client.GetDBTX().Queries.CreateUser(c.client.GetDBTX().Ctx, db.CreateUserParams{
		Username:     "draicor",
		Nickname:     "Draicor",
	...
	*/
	...
	c.client.SocketSend(packets.NewHandshake())
}
...
func (c *Connected) HandleMessage(senderId uint64, payload packets.Payload) {
	...
	if senderId == c.client.Id() {
		...
		case *packets.Packet_Heartbeat:
			...
			c.client.SocketSend(packets.NewHeartbeat())
		case nil:
